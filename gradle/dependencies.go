...
// getBuildDependencies collects all non-transitive dependencies from the
// build classpath, this are basically the dependencies used to build the
// project. It's not clear to me that it's necessary to include these, but
// gradle plugins can end up doing whatever they want to the final artifact.
//
// If we're trying to generate an SBOM *before* build, leave them out for
// now, but include them if we think we need to.
