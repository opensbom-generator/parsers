...
func (m *Yarn) buildDependencies(path string, deps []dependency) ([]meta.Package, error) {
	...
	for _, d := range deps {
		...
		if f, _, ok := strings.Cut(r, "#"); ok {
			mod.PackageDownloadLocation = f
		}
		if mod.PackageDownloadLocation == "" {
			mod.PackageDownloadLocation = fmt.Sprintf(
				"https://www.yarnpkg.com/package/%s", mod.Name,
			)
		}
	...
	for scanner.Scan() {
		...
		if isPk {
			if strings.HasPrefix(text, "  version ") {
				...
				p[i].Name, _, _ = strings.Cut(p[i].Name, "@")
